...
from utils.utils import quat_from_euler_xyz, quat_conjugate, quat_mul, quat_diff_rad
...
class Eurepus:
    def __init__(self):
        ...
        self._pitch_configuration =  False
        self._use_guidance_module = True
        self._policy_rate = 20
        self._interpol_coeff = 0
        self._guidance_max_velocity = 300
        self._pth_file_path = rospack.get_path('eurepus_control') + "/models/eurepus_068.pth"
        self._log_data = False
        self._data_save_path = rospack.get_path('eurepus_control') + "/data/eurepus_068.txt"
        self._ros_rate = 120 # Hz
        self._max_time = 120 #s
        ...
        self._legs_BL = [
            ServoMotor(AnalogIn(self._ADCs_BL[0], ADS.P2), 9, self.LateralMotor_BL_meas_pwm_diff, self.LateralMotor_BL_volt_to_deg, self.LateralMotor_BL_zero_deg_voltage, self.LateralMotor_BL_inverted, self._ros_rate),
            ServoMotor(AnalogIn(self._ADCs_BL[1], ADS.P2), 10, self.InnerTransversalMotor_BL_meas_pwm_diff, self.InnerTransversalMotor_BL_volt_to_deg, self.InnerTransversalMotor_BL_zero_deg_voltage, self.InnerTransversalMotor_BL_inverted, self._ros_rate),
            ServoMotor(AnalogIn(self._ADCs_BL[2], ADS.P2), 11, self.OuterTransversalMotor_BL_meas_pwm_diff, self.OuterTransversalMotor_BL_volt_to_deg, self.OuterTransversalMotor_BL_zero_deg_voltage, self.OuterTransversalMotor_BL_inverted, self._ros_rate)
        ]

        self._legs_FL = [
            ServoMotor(AnalogIn(self._ADCs_FL[0], ADS.P2), 0, self.LateralMotor_FL_meas_pwm_diff, self.LateralMotor_FL_volt_to_deg, self.LateralMotor_FL_zero_deg_voltage, self.LateralMotor_FL_inverted, self._ros_rate),
            ServoMotor(AnalogIn(self._ADCs_FL[1], ADS.P2), 1, self.InnerTransversalMotor_FL_meas_pwm_diff, self.InnerTransversalMotor_FL_volt_to_deg, self.InnerTransversalMotor_FL_zero_deg_voltage, self.InnerTransversalMotor_FL_inverted, self._ros_rate),
            ServoMotor(AnalogIn(self._ADCs_FL[2], ADS.P2), 2, self.OuterTransversalMotor_FL_meas_pwm_diff, self.OuterTransversalMotor_FL_volt_to_deg, self.OuterTransversalMotor_FL_zero_deg_voltage, self.OuterTransversalMotor_FL_inverted, self._ros_rate)
        ]

        self._legs_BR = [
            ServoMotor(AnalogIn(self._ADCs_BR[0], ADS.P2), 6, self.LateralMotor_BR_meas_pwm_diff, self.LateralMotor_BR_volt_to_deg, self.LateralMotor_BR_zero_deg_voltage, self.LateralMotor_BR_inverted, self._ros_rate),
            ServoMotor(AnalogIn(self._ADCs_BR[1], ADS.P2), 7, self.InnerTransversalMotor_BR_meas_pwm_diff, self.InnerTransversalMotor_BR_volt_to_deg, self.InnerTransversalMotor_BR_zero_deg_voltage, self.InnerTransversalMotor_BR_inverted, self._ros_rate),
            ServoMotor(AnalogIn(self._ADCs_BR[2], ADS.P2), 8, self.OuterTransversalMotor_BR_meas_pwm_diff, self.OuterTransversalMotor_BR_volt_to_deg, self.OuterTransversalMotor_BR_zero_deg_voltage, self.OuterTransversalMotor_BR_inverted, self._ros_rate)
        ]

        self._legs_FR = [
            ServoMotor(AnalogIn(self._ADCs_FR[0], ADS.P0), 3, self.LateralMotor_FR_meas_pwm_diff, self.LateralMotor_FR_volt_to_deg, self.LateralMotor_FR_zero_deg_voltage, self.LateralMotor_FR_inverted, self._ros_rate),
            ServoMotor(AnalogIn(self._ADCs_FR[1], ADS.P0), 4, self.InnerTransversalMotor_FR_meas_pwm_diff, self.InnerTransversalMotor_FR_volt_to_deg, self.InnerTransversalMotor_FR_zero_deg_voltage, self.InnerTransversalMotor_FR_inverted, self._ros_rate),
            ServoMotor(AnalogIn(self._ADCs_FR[2], ADS.P0), 5, self.OuterTransversalMotor_FR_meas_pwm_diff, self.OuterTransversalMotor_FR_volt_to_deg, self.OuterTransversalMotor_FR_zero_deg_voltage, self.OuterTransversalMotor_FR_inverted, self._ros_rate)
        ]
        ...
        self._old_pitch = 0
        self._base_vel_buffer = np.array([0.0]*6)
        self._base_pos_buffer = np.array([0.0]*6)
        ...
        self.references = [0, 0, -40, 40, -80, 80, -20, 20, 0, 0, 0, 0, 0, 0, 0]
        self.pole_ref = 0
        self.ref_idx = 0
    ...
    def encoder_callback(self, msg):
        ...
        self._base_rotation = quat_from_euler_xyz( 0, 0, self._base_pos)
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            self._legs_BL[0].low_pass_guidance_second_order_module(30 - self.LateralMotor_BL_sim2real_offset)
            self._legs_BL[1].low_pass_guidance_second_order_module(30 - self.InnerTransversalMotor_BL_sim2real_offset)
            self._legs_BL[2].low_pass_guidance_second_order_module(30 - self.OuterTransversalMotor_BL_sim2real_offset)
            self._legs_FL[0].low_pass_guidance_second_order_module(30 - self.LateralMotor_FL_sim2real_offset)
            self._legs_FL[1].low_pass_guidance_second_order_module(30 - self.InnerTransversalMotor_FL_sim2real_offset)
            self._legs_FL[2].low_pass_guidance_second_order_module(30 - self.OuterTransversalMotor_FL_sim2real_offset)
            self._legs_BR[0].low_pass_guidance_second_order_module(30 - self.LateralMotor_BR_sim2real_offset)
            self._legs_BR[1].low_pass_guidance_second_order_module(30 - self.InnerTransversalMotor_BR_sim2real_offset)
            self._legs_BR[2].low_pass_guidance_second_order_module(30 - self.OuterTransversalMotor_BR_sim2real_offset)
            self._legs_FR[0].low_pass_guidance_second_order_module(30 - self.LateralMotor_FR_sim2real_offset)
            self._legs_FR[1].low_pass_guidance_second_order_module(30 - self.InnerTransversalMotor_FR_sim2real_offset)
            self._legs_FR[2].low_pass_guidance_second_order_module(30 - self.OuterTransversalMotor_FR_sim2real_offset)

            self.apply_motor_angles()

            self.counter += 1
            if self.counter > self._ros_rate*10000:
                break
        ...
        while not rospy.is_shutdown():
            ...
            # Load reference
            self.pole_ref = self._potmeter_ref #self.references[self.ref_idx] * np.pi/180
            if self.counter%(self._ros_rate * 12) == 0:
                self.ref_idx += 1

            orient_ref = quat_from_euler_xyz(0, 0, self.pole_ref)
            ...
            obs = np.array([
                            ...
                            *orient_err,
                            # self._base_pos_filtered - self.pole_ref,
                            0.0,
                            0.0,
                            self._base_vel_filtered,
            ]).astype(np.float32)

            if self._log_data:
                with open(self._data_save_path, 'a') as file:
                    file.write(','.join(map(str, [*obs, self.pole_ref, self._base_pos_filtered, *[leg._measurement_buffer[-1] for leg in self._legs], start_time - self.policy_start_time])) + '\n')
