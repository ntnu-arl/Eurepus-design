...
from utils.utils import quat_from_euler_xyz, quat_conjugate, quat_mul, quat_diff_rad, ang_vel_from_quat, get_euler_xyz
...
class Eurepus:
    def __init__(self):
        ...
        self._policy_rate = 30
        self._interpol_coeff = 0
        self._guidance_max_velocity = 300
        self._pth_file_path = rospack.get_path('eurepus_control') + "/models/eurepus_065.pth"
        self._log_data = True
        self._data_save_path = rospack.get_path('eurepus_control') + "/data/drop_test_065_yaw_-90_deg.txt"
        ...
        self._legs_BL = [
            ServoMotor(AnalogIn(self._ADCs_BL[0], ADS.P1), 9, self.LateralMotor_BL_meas_pwm_diff, self.LateralMotor_BL_volt_to_deg, self.LateralMotor_BL_zero_deg_voltage, self.LateralMotor_BL_inverted, self._ros_rate),
            ServoMotor(AnalogIn(self._ADCs_BL[1], ADS.P1), 10, self.InnerTransversalMotor_BL_meas_pwm_diff, self.InnerTransversalMotor_BL_volt_to_deg, self.InnerTransversalMotor_BL_zero_deg_voltage, self.InnerTransversalMotor_BL_inverted, self._ros_rate),
            ServoMotor(AnalogIn(self._ADCs_BL[2], ADS.P1), 11, self.OuterTransversalMotor_BL_meas_pwm_diff, self.OuterTransversalMotor_BL_volt_to_deg, self.OuterTransversalMotor_BL_zero_deg_voltage, self.OuterTransversalMotor_BL_inverted, self._ros_rate)
        ]

        self._legs_FL = [
            ServoMotor(AnalogIn(self._ADCs_FL[0], ADS.P1), 0, self.LateralMotor_FL_meas_pwm_diff, self.LateralMotor_FL_volt_to_deg, self.LateralMotor_FL_zero_deg_voltage, self.LateralMotor_FL_inverted, self._ros_rate),
            ServoMotor(AnalogIn(self._ADCs_FL[1], ADS.P1), 1, self.InnerTransversalMotor_FL_meas_pwm_diff, self.InnerTransversalMotor_FL_volt_to_deg, self.InnerTransversalMotor_FL_zero_deg_voltage, self.InnerTransversalMotor_FL_inverted, self._ros_rate),
            ServoMotor(AnalogIn(self._ADCs_FL[2], ADS.P1), 2, self.OuterTransversalMotor_FL_meas_pwm_diff, self.OuterTransversalMotor_FL_volt_to_deg, self.OuterTransversalMotor_FL_zero_deg_voltage, self.OuterTransversalMotor_FL_inverted, self._ros_rate)
        ]

        self._legs_BR = [
            ServoMotor(AnalogIn(self._ADCs_BR[0], ADS.P1), 6, self.LateralMotor_BR_meas_pwm_diff, self.LateralMotor_BR_volt_to_deg, self.LateralMotor_BR_zero_deg_voltage, self.LateralMotor_BR_inverted, self._ros_rate),
            ServoMotor(AnalogIn(self._ADCs_BR[1], ADS.P1), 7, self.InnerTransversalMotor_BR_meas_pwm_diff, self.InnerTransversalMotor_BR_volt_to_deg, self.InnerTransversalMotor_BR_zero_deg_voltage, self.InnerTransversalMotor_BR_inverted, self._ros_rate),
            ServoMotor(AnalogIn(self._ADCs_BR[2], ADS.P1), 8, self.OuterTransversalMotor_BR_meas_pwm_diff, self.OuterTransversalMotor_BR_volt_to_deg, self.OuterTransversalMotor_BR_zero_deg_voltage, self.OuterTransversalMotor_BR_inverted, self._ros_rate)
        ]

        self._legs_FR = [
            ServoMotor(AnalogIn(self._ADCs_FR[0], ADS.P2), 3, self.LateralMotor_FR_meas_pwm_diff, self.LateralMotor_FR_volt_to_deg, self.LateralMotor_FR_zero_deg_voltage, self.LateralMotor_FR_inverted, self._ros_rate),
            ServoMotor(AnalogIn(self._ADCs_FR[1], ADS.P2), 4, self.InnerTransversalMotor_FR_meas_pwm_diff, self.InnerTransversalMotor_FR_volt_to_deg, self.InnerTransversalMotor_FR_zero_deg_voltage, self.InnerTransversalMotor_FR_inverted, self._ros_rate),
            ServoMotor(AnalogIn(self._ADCs_FR[2], ADS.P2), 5, self.OuterTransversalMotor_FR_meas_pwm_diff, self.OuterTransversalMotor_FR_volt_to_deg, self.OuterTransversalMotor_FR_zero_deg_voltage, self.OuterTransversalMotor_FR_inverted, self._ros_rate)
        ]
    ...
    def mocap_callback(self, msg):
        ...
        # base_euler = get_euler_xyz(self._base_rotation)
        # base_rotation_no_yaw = quat_from_euler_xyz(base_euler[0], base_euler[1], 0)
        ...
        # Load reference
        orient_ref = quat_from_euler_xyz(0, 0, -90 * np.pi/180)
        orient_err = quat_mul(quat_conjugate(orient_ref), self._base_rotation)
        self.base_ang_vel = ang_vel_from_quat(self.old_orient_err, orient_err, 1/self._ros_rate)
        ...
        if self._log_data:
            with open(self._data_save_path, 'a') as file:
                file.write(','.join(map(str, [*obs, self.pole_ref, self._base_rotation, *[leg._measurement_buffer[-1] for leg in self._legs], start_time - self.policy_start_time])) + '\n')
        ...
        interpol_coeff = np.exp(-orr_err**2 / 0.02) 
